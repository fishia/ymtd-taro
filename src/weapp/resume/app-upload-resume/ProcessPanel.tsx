...
const ProcessPanel: FC<IProcessPanelProps> = props => {
  const { onJumpError = noop, isFinished = false, resume } = props
  ...
  return (
    <View className="process-panel">
      ...
      <View className="process-panel__tips">
        <View className="process-panel__tips__row">
          {isFinished ? '解析成功' : '正在上传'}
        </View>
        <View className="process-panel__tips__row">{isFinished ? '' : '正在上传/解析...'}</View>
      </View>
      ...
      {isFinished ? (
        <View className="process-panel__action">
          <Button
            openType="launchApp"
            onError={onJumpError}
            appParameter={JSON.stringify(resume)}
            onLaunchapp={() => void navigateTo({ url: '/weapp/pages/job/index' })}
            className="process-panel__button"
            round
          >
            返回医脉同道
          </Button>
