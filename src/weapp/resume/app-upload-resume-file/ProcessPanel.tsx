...
const ProcessPanel: FC<IProcessPanelProps> = props => {
  const { onJumpError = noop, isFinished = false, result } = props
  ...
  return (
    <View className="process-panel">
      ...
      <View className="process-panel__tips">
        <View className="process-panel__tips__row">{isFinished ? '上传成功' : '正在上传'}</View>
      </View>
      ...
      {isFinished ? (
        <View className="process-panel__action">
          <Button
            openType="launchApp"
            onError={onJumpError}
            appParameter={JSON.stringify(result)}
            onLaunchapp={() => void switchTab({ url: '/weapp/pages/job/index' })}
